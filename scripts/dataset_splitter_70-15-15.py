...
def split_yolo_dataset(source_images_dir, source_labels_dir, output_dir, train_ratio=0.70, val_ratio=0.15):
    # Calculate test ratio automatically
    test_ratio = 1.0 - train_ratio - val_ratio
    
    print(f"Starting dataset split: {train_ratio*100}% Train | {val_ratio*100}% Val | {test_ratio*100}% Test")

    # Define the new folder structure for YOLO
    dirs_to_create = [
    ...
    # Get all images
    # We check for standard image formats just in case
    valid_extensions = ('.jpg', '.jpeg', '.png', '.bmp')
    images = [f for f in os.listdir(source_images_dir) if f.lower().endswith(valid_extensions)]
    
    # Shuffle the dataset to ensure random distribution
    random.seed(42) # Keeps the randomness the same if you re-run the script
    ...
    def copy_files(file_list, split_name):
        ...
        for img_name in file_list:
            ...
            # YOLO labels have the exact same name as the image, just with a .txt extension
            ...
            # If the label exists (even if it's an empty file for a background image), copy it
...
if __name__ == '__main__':
    ...
    RAW_IMAGES_FOLDER = 'C:/path/to/your/label_studio_export/images'
    RAW_LABELS_FOLDER = 'C:/path/to/your/label_studio_export/labels'
    
    # Where you want the final, split dataset to be built
    FINAL_OUTPUT_FOLDER = 'C:/path/to/your/ulang_dataset_split'
